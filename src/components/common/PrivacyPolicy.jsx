...
const PrivacyPolicy = ({ isDarkTheme }) => {
  ...
  usePageTitle('page_titles.privacy_policy', t)
  ...
  const sections = [
    'controller', 'service_presentation', 'data_collected', 'purposes',
    'legal_basis', 'recipients', 'retention', 'security', 'local_storage',
    'rights', 'complaint', 'changes'
  ]
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
        <header className="space-y-2">
          ...
          <p className={`text-sm md:text-base ${isDarkTheme ? 'text-slate-300' : 'text-slate-600'}`}>
            {t('privacy_policy.updated')}
          </p>
        </header>

        {sections.map((key) => (
          <section key={key} className={sectionClass}>
            <h2 className={titleClass}>{t(`privacy_policy.${key}_title`)}</h2>
            <p className={`${bodyClass} whitespace-pre-line`}>{t(`privacy_policy.${key}_body`)}</p>
          </section>
        ))}
      </div>
    </div>
